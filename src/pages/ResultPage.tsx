...
export const ResultPage = () => {
  ...
  const getLevelInfo = (level: string) => {
    const levels = {
      SSS: { emoji: '👑', text: '오늘 주신이 강림했다', color: 'from-yellow-400 to-orange-500' },
      S: { emoji: '🔥', text: '오늘 개잘마셔', color: 'from-red-400 to-pink-500' },
      A: { emoji: '👍', text: '그럭저럭 ㄱㄴ', color: 'from-green-400 to-blue-500' },
      B: { emoji: '🤔', text: '적당히 해', color: 'from-blue-400 to-purple-500' },
      C: { emoji: '🏠', text: '집에서 캔맥이나', color: 'from-gray-400 to-gray-600' },
    ...
    <>
      <PageContainer showProgress currentStep={5} totalSteps={5}>
        ...
        >
          ...
          <div className="text-3xl font-bold mt-4">평소의 {result.percentage}% 파워!</div>
        </motion.div>
        ...
        >
          <h3 className="text-xl font-bold text-gray-800 mb-4">📱 오늘의 추천 주종</h3>
          <div className="space-y-3">
            {result.recommendations.map((rec, index) => (
              <div key={index} className="flex justify-between items-center p-3 bg-gray-50 rounded-xl">
                ...
                <div className="text-lg font-bold text-pink-500">{rec.amount}</div>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          className="bg-white rounded-2xl p-6 shadow-lg"
        >
          <h3 className="text-xl font-bold text-gray-800 mb-4">⏰ 음주 스케줄</h3>
        ...
        >
          <h3 className="text-lg font-bold text-gray-800 mb-3">🚗 집 가는 시간</h3>
